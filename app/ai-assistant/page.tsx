...
export default function AIAssistantPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 pb-24">
      {/* Header */}
      <div className="rounded-3xl bg-gradient-to-r from-emerald-900 via-teal-900 to-green-950 text-white p-6 sm:p-8 shadow-xl border border-emerald-500/30 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          <DhartiMaaAvatar size="xl" className="border-4 border-yellow-400 shrink-0" />
          <div className="space-y-1.5">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/30 border border-emerald-400/40 text-yellow-300 text-xs font-semibold">
        ...
        <div className="flex items-center gap-2 self-start md:self-auto shrink-0">
          <button
            ...
            className="px-3.5 py-2.5 rounded-2xl bg-white/10 hover:bg-white/20 border border-emerald-400/40 text-white text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm"
          >
      ...
      {showKeyConfig && (
        <div className="p-4 rounded-2xl bg-emerald-900/90 text-white border border-emerald-500/40 text-xs space-y-2 animate-fadeIn shadow-md">
          <div className="flex items-center justify-between font-bold">
            <span className="flex items-center gap-2 text-emerald-200">
              <Cpu className="w-4 h-4 text-emerald-300" />
              Connected Model: OpenAI GPT-4o
            </span>
            <span className="px-2 py-0.5 rounded-full bg-emerald-700/80 text-emerald-200 text-[10px] border border-emerald-400/30">
              Active Connection
            </span>
          </div>
          ...
          <div className="flex gap-2 max-w-md">
            <input
              ...
              className="flex-1 bg-emerald-950/80 border border-emerald-600/60 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:ring-1 focus:ring-emerald-400 font-mono"
            />
            {customApiKey && (
              <span className="text-emerald-300 text-xs self-center flex items-center gap-1 font-semibold">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <span className="font-bold text-stone-700 dark:text-stone-300 self-center mr-1">Quick Prompts:</span>
        {sampleQuestions.map((chip, idx) => (
          <button
            key={idx}
            onClick={() => handleSend(chip)}
            className="px-3.5 py-1.5 rounded-full bg-white dark:bg-emerald-900 border border-stone-200 dark:border-emerald-700 hover:bg-emerald-50 text-stone-800 dark:text-stone-200 font-medium transition-all shadow-xs"
          >
            {chip}
          </button>
        ))}
      ...
      <div className="bg-white dark:bg-emerald-950 rounded-3xl border border-stone-200 dark:border-emerald-800 shadow-md flex flex-col h-[520px] overflow-hidden">
        {/* Messages */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4 text-xs">
              ...
              <div
                className={`max-w-[80%] rounded-2xl p-4 shadow-xs leading-relaxed whitespace-pre-line ${
                  m.sender === "user"
                    ...
                    : "bg-stone-50 dark:bg-emerald-900/60 text-stone-900 dark:text-stone-100 border border-stone-200 dark:border-emerald-800 rounded-bl-none"
                }`}
              >
                <div>{m.text}</div>
                <div className="flex items-center justify-between gap-2 mt-2 pt-1 border-t border-emerald-100/40 dark:border-emerald-800/40">
                  {m.provider ? (
                  ...
                  <span className={`text-[10px] ${m.sender === "user" ? "text-emerald-200" : "text-stone-400"}`}>
                    {m.time}
                  </span>
                </div>
              </div>
          ...
          {loading && (
            <div className="flex items-center gap-2 p-3 bg-stone-50 dark:bg-emerald-900/40 rounded-2xl w-fit text-stone-600 dark:text-stone-300 text-xs">
        ...
        {/* Input Bar */}
        <div className="p-4 bg-stone-50 dark:bg-emerald-900/40 border-t border-stone-200 dark:border-emerald-800 flex items-center gap-2">
          <input
            type="text"
            value={input}
            ...
            className="flex-1 px-4 py-3 rounded-xl border border-stone-200 dark:border-emerald-800 bg-white dark:bg-emerald-950 text-stone-900 dark:text-stone-100 text-xs focus:ring-2 focus:ring-emerald-600 focus:outline-none"
          />
          <button
            onClick={() => handleSend()}
            disabled={!input.trim() || loading}
            className="px-5 py-3 bg-emerald-800 hover:bg-emerald-900 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 disabled:opacity-50 transition-colors shadow"
          >
      ...
      <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-[11px] text-amber-900 dark:text-amber-200 leading-relaxed text-center">
